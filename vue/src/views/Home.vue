...
<!-- 顶部标题栏 -->
<header class="h-20 bg-dark-blue-transparent border-b border-blue-800 flex items-center justify-between px-6 z-10">
<div class="flex items-center">
<i class="fas fa-subway text-orange text-2xl mr-3"></i>
<h1 class="text-2xl font-bold">地铁隧道监测系统</h1>
</div>
...
:key="index"
:class="[
'flex items-center px-4 py-2 cursor-pointer rounded-md transition-all duration-200',
activeNavIndex === index ? 'bg-blue-900 text-white' : 'text-gray-300 hover:bg-blue-900/50'
]"
...
item.icon,
'text-xl mr-2',
activeNavIndex === index ? 'text-orange' : 'text-gray-400'
]"
></i>
<span>{{ item.name }}</span>
</li>
...
<div class="flex-1 p-4 relative">
  <div class="w-full h-full flex gap-4">
    ...
    <div class="w-[400px] flex flex-col gap-4">
      <!-- 沉降监测数据 -->
      <div class="flex-1 bg-dark-blue-light rounded-lg border border-blue-800 shadow-xl p-4 flex flex-col">
      ...
      <!-- 事件日志 -->
      <div class="h-[300px] bg-dark-blue-light rounded-lg border border-blue-800 shadow-xl p-4">
        <div class="flex items-center mb-3">
          ...
          <div class="ml-4 flex">
            <span class="px-2 py-1 rounded-md text-xs mr-2 bg-red-900 text-red-300">告警</span>
            <span class="px-2 py-1 rounded-md text-xs mr-2 bg-yellow-900 text-yellow-300">提示</span>
            <span class="px-2 py-1 rounded-md text-xs bg-blue-900 text-blue-300">操作</span>
          </div>
        </div>
        <div class="h-[calc(100%-40px)] overflow-hidden relative" ref="eventLogContainer">
          <TransitionGroup 
            name="log-slide"
            tag="div"
          ...
          >
            <div
              ...
              :class="[
                ...
                log.type === 'alert' ? 'bg-red-900/50' :
                log.type === 'warning' ? 'bg-yellow-900/30' : 'bg-blue-900/30'
              ]"
            >
              <i :class="[
                'fas',
                log.type === 'alert' ? 'fa-exclamation-triangle text-red-500' :
                log.type === 'warning' ? 'fa-exclamation-circle text-yellow-500' : 'fa-info-circle text-blue-500',
                'mr-3'
              ]"></i>
    ...
    <!-- 右侧大屏显示区域 -->
    <div ref="dashboardContainer" class="flex-1 bg-dark-blue-light rounded-lg border border-blue-800 shadow-xl overflow-hidden">
      <div class="w-full h-full p-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center space-x-4">
            ...
            >
              <i :class="['fas', isFullscreen ? 'fa-compress-arrows-alt' : 'fa-expand-arrows-alt', 'mr-2']"></i>
              {{ isFullscreen ? '退出全屏' : '全屏显示' }}
            </button>
            <button class="px-4 py-2 bg-blue-900/50 hover:bg-blue-900 rounded-button flex items-center transition-all duration-200">
              <i class="fas fa-sync-alt mr-2"></i>
              刷新
            </button>
          </div>
        </div>
        <div class="w-full h-[calc(100%-4rem)] bg-dark-blue rounded-lg border border-blue-900/50 p-4">
...
<!-- 右侧数据面板 -->
<aside class="w-[320px] bg-dark-blue-light border-l border-blue-800 overflow-y-auto custom-scrollbar">
<!-- 列车运行信息 -->
...
<span class="text-2xl font-bold text-cyan-400">{{ trainSpeed }} <span class="text-sm">km/h</span></span>
</div>
...
<div class="relative w-full h-8 bg-dark-blue rounded-md overflow-hidden border border-blue-800">
...
:style="{ left: `${(trainPosition / 1000) * 100}%`, width: '30px' }">
...
<span class="text-cyan-400">{{ trainDirection === 1 ? '正向' : '反向' }}</span>
</div>
...
trainDirection === 1 ? 'text-orange' : 'text-gray-600 rotate-180'
]"></i>
</div>
</div>
</div>
...
temperature < 15 ? 'text-blue-400' : 'text-green-500'
]">{{ temperature }}°C</span>
</div>
...
temperature < 15 ? 'bg-blue-400' : 'bg-green-500'
]"
...
<span :class="[
'text-xl font-bold',
humidity > 80 ? 'text-blue-400' :
humidity < 30 ? 'text-yellow-500' : 'text-green-500'
]">{{ humidity }}%</span>
</div>
<div class="w-full bg-dark-blue rounded-full h-2">
<div
...
humidity < 30 ? 'bg-yellow-500' : 'bg-green-500'
]"
...
<span :class="[
'text-xl font-bold',
gasConcentration > 0.8 ? 'text-red-500' :
gasConcentration > 0.5 ? 'text-yellow-500' : 'text-green-500'
...
:class="[
'h-2 rounded-full transition-all duration-500',
gasConcentration > 0.8 ? 'bg-red-500' :
gasConcentration > 0.5 ? 'bg-yellow-500' : 'bg-green-500'
]"
...
<span :class="[
'text-xl font-bold',
airQuality > 150 ? 'text-red-500' :
airQuality > 100 ? 'text-yellow-500' : 'text-green-500'
]">{{ airQuality }}</span>
</div>
<div class="w-full bg-dark-blue rounded-full h-2">
<div
:class="[
'h-2 rounded-full transition-all duration-500',
airQuality > 150 ? 'bg-red-500' :
airQuality > 100 ? 'bg-yellow-500' : 'bg-green-500'
]"
...
xAxis: {
type: 'category',
data: ['K5+100', 'K5+150', 'K5+200', 'K5+250', 'K5+300', 'K5+350', 'K5+400', 'K5+450'],
axisLine: {
lineStyle: {
color: '#2a385c'
}
...
trainSpeed.value = Math.max(0, Math.min(120, trainSpeed.value + randomSpeedChange));
// 更新环境数据
temperature.value = Math.max(15, Math.min(35, temperature.value + (Math.random() - 0.5)));
humidity.value = Math.max(30, Math.min(90, humidity.value + (Math.random() - 0.5) * 2));
gasConcentration.value = Math.max(0.1, Math.min(1.2, gasConcentration.value + (Math.random() - 0.5) * 0.1));
airQuality.value = Math.max(50, Math.min(200, airQuality.value + (Math.random() - 0.5) * 5));
// 更新沉降数据
settlementData.value = settlementData.value.map(item => {
const newValue = Math.max(1, Math.min(8, item.value + (Math.random() - 0.5) * 0.2));
// 添加告警日志
if (newValue > 5 && item.value <= 5) {
addEventLog('alert', `${item.name}区段沉降超过警戒值${newValue.toFixed(1)}mm`);
}
